...
# at .../deckhand-sandbox/domains/). Regenerated each deploy, so links never rot.
{
  echo '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">'
  echo '<title>Deckhand deliverables</title>'
  echo '<style>body{font:15px/1.6 -apple-system,Segoe UI,Roboto,sans-serif;margin:2.5em auto;max-width:760px;color:#0f172a}a{color:#0b5394}</style>'
  echo '</head><body><h1>Deckhand deliverables</h1><ul>'
  while IFS= read -r f; do
    rel="${f#"$work/"}"
    echo "<li><a href=\"/deckhand-sandbox/$rel\">$rel</a></li>"
  done < <(find "$work/domains" -name 'report.html' | sort)
  echo '</ul><p><small>Auto-generated by the deploy workflow.</small></p></body></html>'
